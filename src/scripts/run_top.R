...
#select winning alternative from the tournament
ToAlternative <- function(lpairs){
  alts <- c()
  for(v in lpairs){
    alts <- c(alts, v[1], v[2])
  }
...
mse_ktdist <- function(d)
{
  ...
  for (i in 1:nrow(Q.test.temp)) {
    ...
    Q.test.temp[i,tr] <- ToAlternative(ordered_pairs)
  ...
  for (i in 1:nrow(Q.test.temp)) {
    ...
    #   
    # }
    Q.test.dist.temp[i,tr] <- TALoss(Q.test.temp[i,]$options, Q.test.temp[i,tr][[1]]) 
...
#zero-one loss (check if alt equals rank[1])
...
for(tr in c('RankRank', 'RankTop', 'RankNone', 'TopRank', 'TopTop', 'TopNone'))
{
  ...
  alpha_0 <- 0.55
  beta_0 <- 0.1
  for(alpha in seq(0.55,0.90,0.05))
  {
    for(beta in seq(0.1,0.45,0.05))
    {
      for (i in 1:nrow(Q.train)) {
        ...
        #tempstr <- paste0(ToRank( ordered_pairs), collapse = ", ")
        Q.train[i,tr] <- TALoss(Q.train[i,]$options, ToAlternative(ordered_pairs))
...
for(tr in c('RankRank', 'RankTop', 'RankNone', 'TopRank', 'TopTop', 'TopNone'))
{
  ...
  for (i in 1:nrow(Q.test)) {
    ...
    #tempstr <- paste0(ToRank( ordered_pairs), collapse = ", ")
    Q.test[i,tr] <- ToAlternative(ordered_pairs)
...
for(tr in c('RankRank', 'RankTop', 'RankNone', 'TopRank', 'TopTop', 'TopNone'))
{
  for (i in 1:nrow(Q.test)) {
    ...
    Q.test.dist[i,tr] <- TALoss(Q.test[i,]$options, Q.test[i,tr][[1]]) 
    #print(KTDist_new(Q.test[i,]$options, list_pairs) )
  }
}
